...
const SingleTrek: React.FC<props> = ({ id }) => {
  ...
  const activitiesWithImages =
    singleTrek?.data?.specificTrek?.trekHighlights &&
    singleTrek?.data?.specificTrek?.trekHighlights?.map(
  ...
  const itineraryWithImages =
    singleTrek?.data?.specificTrek?.trekItinerary?.map(
  ...
  return (
    <div className="lg:px-20 px-4 lg:my-16 my-4">
        ...
        <section className="flex lg:flex-row flex-col items-center justify-between">
          <div className="flex gap-20 flex-wrap max-sm:gap-8 items-center sm:w-1/5 w-full">
            ...
            <main className="">
              ...
              <p>
                ${trip.price} <span className="text-gray-500">/per person</span>
              </p>
            </main>
          </div>
      ...
      <QuoteModal
        ...
        Title={trip.title}
        type={singleTrek?.data?.specificTrek?.type}
        trekId={singleTrek?.data?.specificTrek?._id}
      />
      <MainSlider
        gallery={singleTrek.data.specificTrek?.gallery}
        thumbnail={singleTrek.data.specificTrek?.thumbnail}
      />
      ...
      {singleTrek?.data?.specificTrek?.routeMap && (
        <div className="w-full flex flex-col gap-8 items-center justify-start">
          ...
          <div className="w-full h-[700px]">
            <Image
              src={singleTrek?.data?.specificTrek?.routeMap}
              alt="image"
              height={1000}
              width={1000}
      ...
      <div className="bg-primary/10 w-full my-16 gap-4 flex flex-col items-center justify-center py-12">
        ...
        <p className="w-4/5 text-justify lg:text-base text-sm my-4">
          ...
          your personal tastes and interests. Whether it&apos; an exclusive
          cultural experience, a breathtaking helicopter tour, or a relaxing
          retreat, we're here to bring your vision to life with seamless
          planning and expert guidance. Enjoy a journey that is crafted uniquely
          for you, ensuring every moment is unforgettable.
        </p>
