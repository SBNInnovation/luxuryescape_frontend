...
export const getTours = async (page:number, limit:number,country:string) => {
    try{
        const res=await axios.get(`${process.env.NEXT_PUBLIC_BASE_URL}/tour/get/selected?page=${page}&limit=${limit}&search=&filter=&sort=asc&country=${country}&activation=true`)
        return res.data
    }catch(err){
        console.log(err)
    }
