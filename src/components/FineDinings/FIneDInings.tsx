...
import { getFine } from '@/services/dinings';
...
const FIneDinings: React.FC = () => {
  ...
  const { data: accomData, isLoading } = useQuery({
    queryKey: ['finedinings'],
    queryFn: () => getFine(1, 1000, ''), // Fetch a large number to get all accommodations
  });
    ...
    return accomData.data.formattedData.filter((hotel: Accommodation) => {
      ...
      if (filterStarRating !== 'all') {
        ...
        if (filterRating === 6) {
          ...
          matchesStarRating = hotel.rating === 6 || hotel.isPremium;
        } else if (filterRating === 7) {
          // Boutique - check for rating 7
          matchesStarRating = hotel.rating === 7;
        } else if (filterRating === 8) {
          // Premium Boutique - check for rating 8
          matchesStarRating = hotel.rating === 8;
        } else {
          // Regular 4 or 5 star ratings
          matchesStarRating = hotel.rating === filterRating;
        }
      }
  ...
  return (
    <div className="min-h-screen">
      <div className="relative h-[500px] overflow-hidden">
        <Image
          src="https://images.unsplash.com/photo-1631030960702-5c6db58fbd0d?q=80&w=2940&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
        ...
        >
          ...
          >
            Luxury <span className="text-primary">Fine Dinings</span> in Nepal
          </h1>
          <p className="lg:text-xl text-lg text-white/90 font-light text-center max-w-2xl">
            Indulge in exquisite fine dining where culinary artistry meets
            Himalayan elegance
          </p>
        </div>
      </div>
          ...
          {paginatedHotels?.map((hotel: Accommodation) => (
            ...
            >
              <div className="relative h-64">
                ...
                {(hotel?.isPremium || hotel.rating === 6) && (
              ...
              <div className="px-4 pt-2 pb-4 flex flex-col h-1/2">
                ...
                <div className="flex items-center justify-end mt-auto">
                  <Link href={`/fine-dinings/${hotel.slug}`}>
                    <Button
                      size="sm"
