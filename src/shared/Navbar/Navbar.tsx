      ...
      <div className="shadow-sm top-0 flex flex-col items-center justify-between px-4 md:px-16 py-2 bg-primary/5 w-full z-[9999]">
        <div className="w-full flex justify-between items-center font-medium uppercase text-sm tracking-wide">
          ...
          <div className="flex gap-2 md:hidden">
            ...
            >
              {isMenuOpen ? (
                <IoMdClose size={26} className="text-primary" />
              ) : (
                <HiMenu size={26} className="text-primary" />
              )}
            </Button>
          </div>
          ...
          <div className="hidden md:flex items-center gap-8">
            ...
            <Link href="/tailor-made">
              <Button
                className="bg-primary rounded-sm px-8 text-white"
                size="sm"
              >
                Customize trip
              </Button>
        ...
        <div className="flex gap-4 lg:gap-8 flex-wrap justify-center">
          {nav.map((item) =>
            item.isDropdown ? (
              <Dropdown
                key={item.title}
                className="bg-primary/90 text-white rounded-sm"
              ...
              >
                <DropdownTrigger>
                  <div className="group font-normal text-sm text-gray-600 uppercase relative cursor-pointer tracking-wider flex items-center gap-2">
                    ...
                    <span className="absolute -bottom-0 left-0 h-[2px] bg-primary transition-all w-0 group-hover:w-4/5"></span>
                  </div>
                </DropdownTrigger>
      ...
      >
        <div className="flex flex-col gap-2 p-6 w-full">
          ...
          <div className="flex gap-4 justify-center mb-6">
            <Button
              isIconOnly
              className="h-8 w-8 border border-gray-200 hover:border-primary bg-white text-primary rounded-full flex items-center justify-center hover:bg-primary transition duration-300 hover:text-white"
            >
              <FaFacebookF size={16} />
            </Button>
            <Button
          ...
          {nav.map((item, index) => (
            <div
              key={index}
              className="w-full border-b border-gray-100 pb-3 mb-3"
            >
              {item.isDropdown ? (
                <div>
                  <div
                  ...
                  >
                    <p className="font-normal text-gray-600 uppercase text-base">
                      {item.title}
                    </p>
                    <IoMdArrowDropdown
                      size={20}
                ...
                <Link href={item.link} onClick={() => setIsMenuOpen(false)}>
                  <p
                    className={`font-normal uppercase text-base ${isActive(item.link) ? 'text-primary' : 'text-gray-600'}`}
                  >
                    {item.title}
                  </p>
                </Link>
          ...
          <div className="mt-4">
            <Link href="/tailor-made" onClick={() => setIsMenuOpen(false)}>
              <Button
                className="bg-primary w-full rounded-sm py-6 text-white"
                size="md"
              >
                Customize trip
              </Button>
