...
export const getAffiliates=async()=>{
    try{
        const res=await axios.get(`https://going-nepal-adventure.onrender.com/api/v1/resources/get-affiliates`)
        return res.data
    }catch(err){
        console.log(err)
    }
...
const Footer = () => {
    ...
    const nationalData=affiliatesData?.affiliates?.filter((item:any)=>item.type==="national") // eslint-disable-line @typescript-eslint/no-explicit-any
    const interNationalData=affiliatesData?.affiliates?.filter((item:any)=>item.type==="international") // eslint-disable-line @typescript-eslint/no-explicit-any

    return (
        <>
            <main className='flex flex-col my-4 mt-12 gap-4 w-full items-center justify-center'>
                <div className='flex flex-col gap-8 w-full items-center justify-center'>
                    <h1 className={`lg:text-4xl md:text-3xl text-2xl text-primary ${antic.className}`}>National Affiliates</h1>
                    <div className='flex items-center flex-wrap justify-center md:justify-evenly gap-6 px-4 md:px-8 lg:px-[12rem] w-full'>
                        {nationalData?.map((item:{logo:string},index:number)=>{
                            return(
                                <Image key={index} src={item?.logo} alt="image" height={1000} width={1000} className='object-contain size-20 md:size-24 rounded-md'/>
                            )
                        })}
                    </div>
                </div>
                <div className='flex flex-col gap-8 mt-8 w-full items-center justify-center'>
                    <h1 className={`lg:text-4xl md:text-3xl text-2xl text-primary ${antic.className}`}>International Affiliates</h1>
